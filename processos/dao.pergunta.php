	...
	class PerguntaDAO extends Conexao{
		...
		function ListarPergunta(){
		 ...
		 function ListarDataHora1(){
			$resultado = $this->query("SELECT perguntas FROM pergunta WHERE TIME('07:00:00') LIMIT 1 ");
			$resultado->setFetchMode(PDO::FETCH_ASSOC);
		...
		function ListarDataHora2(){
			$resultado = $this->query("SELECT perguntas FROM pergunta WHERE TIME('13:00:00') LIMIT 1 ");
			$resultado->setFetchMode(PDO::FETCH_ASSOC);
			$pergunta = $resultado->fetchAll();
		...
		function ListarDataHora3(){
			$resultado = $this->query("SELECT perguntas FROM pergunta WHERE TIME('19:00:00') LIMIT 1 ");
			$resultado->setFetchMode(PDO::FETCH_ASSOC);
			$pergunta = $resultado->fetchAll();
		...
		function ListarDataHora4(){
			$resultado = $this->query("SELECT perguntas FROM pergunta WHERE TIME('23:00:00') LIMIT 1 ");
			$resultado->setFetchMode(PDO::FETCH_ASSOC);
			$pergunta = $resultado->fetchAll();
